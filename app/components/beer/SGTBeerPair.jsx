// app/components/beer/BeerCameraOrUpload.jsx
"use client";
...
export default function BeerCameraOrUpload({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoStartOnMount, mounted, hasMediaDevices, isSecure]);
  ...
  /* ---------- Search, scoring, selection ---------- */

  const [query, setQuery] = useState("");
  const [activeStyle, setActiveStyle] = useState("");

  // Pizza selection (TOP DROPDOWN)
  const [pizzaLocal, setPizzaLocal] = useState(selectedPizzaName || "");

  // keep local in sync if parent updates selectedPizzaName
  useEffect(() => {
    setPizzaLocal(selectedPizzaName || "");
  }, [selectedPizzaName]);

  // notify parent when user changes local pizza
  useEffect(() => {
    if (pizzaLocal !== (selectedPizzaName || "")) {
      onSelectPizza?.(pizzaLocal);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pizzaLocal, selectedPizzaName]);
  ...
  const scoreBeer = (beer) => {
    const base = 20;
    const style = normalize(beer.style || "");
    const styleTokens = tokenize(style);

    let maxSim = 0;
    if (styleTokensList.length) {
      for (const sTok of styleTokensList) {
        maxSim = Math.max(maxSim, similarity(styleTokens, sTok));
      }
    }
    const styleScore = Math.round(maxSim * 70); // up to 70

    // ABV sweet spot (4.0–6.5) = +5
    ...
    let total = base + styleScore + abvScore + ibuScore + notesBonus;
  ...
  const filteredResults = useMemo(() => {
    ...
    return (results || []).filter((b) => {
      ...
      const styleOk =
        !activeStyle || (b.style || "").toLowerCase() === activeStyle.toLowerCase();
      return qOk && styleOk;
    });
  ...
  // Score + rank (best first)
  const scoredResults = useMemo(() => {
    const withScores = filteredResults.map((b) => ({
      beer: b,
      score: scoreBeer(b),
      key: makeKey(b),
    }));
    withScores.sort((a, b) => b.score - a.score);
    return withScores.map((x, idx) => ({ ...x, rank: idx + 1 }));
  }, [filteredResults, styleTokensList.length]);

  // Selected beer
  const [selectedBeerKey, setSelectedBeerKey] = useState("");
  const selectedBeerObj = useMemo(() => {
    if (!selectedBeerKey) return null;
    return scoredResults.find((i) => i.key === selectedBeerKey)?.beer || null;
  }, [selectedBeerKey, scoredResults]);

  // Per-item AI advice state
  const [advice, setAdvice] = useState({}); // { [rankIndex]: { loading, open, markdown, error } }
  ...
  async function askAIFor(idx) {
    ...
    setAdvice((s) => ({
      ...s,
      [idx]: { ...(s[idx] || {}), loading: true, error: "", open: true },
    }));
    try {
      ...
      if (onAskAI) {
        ...
        const res = await fetch("/api/beer-advice", {
          ...
          body: JSON.stringify({
            beer: item,
            pizzaName: effectivePizzaName || null,
          }),
        });
      ...
      setAdvice((s) => ({
        ...
        [idx]: {
          ...(s[idx] || {}),
          loading: false,
          markdown: payload.markdown || "No advice returned.",
          error: "",
          open: true,
        },
      }));
  ...
  function toggleAdvice(idx) {
    setAdvice((s) => ({
      ...s,
      [idx]: { ...(s[idx] || {}), open: !s[idx]?.open },
    }));
  }

  // --- Shared confirm action (used by top "Pair Beer + Pizza")
  function handleConfirm() {
    if (!selectedBeerObj || !effectivePizzaName) return;
    onConfirmPairing?.({
      beer: selectedBeerObj,
      pizzaName: effectivePizzaName,
    });
  }
    ...
    <div className="mt-2">
      ...
      <div
        ...
        style={{
          width: `${score}%`,
          background:
            "linear-gradient(90deg, rgba(23,80,58,0.85) 0%, rgba(242,178,58,0.9) 100%)",
        }}
      />
    </div>
  );
  ...
  const BeerCard = ({ item, idx }) => {
    ...
    return (
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") setSelectedBeerKey(key);
        }}
      >
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              ...
              title={
                effectivePizzaName
                  ? `Ask why this pairs with ${effectivePizzaName}`
                  : "Ask AI for tasting + pairing guidance"
              }
            >
              {s.loading ? (
                <>
              ...
              <div className="mt-2 rounded-xl border border-gray-200 bg-white p-3 sm:p-4">
                <div className="text-[13.5px] sm:text-[14px] md:text-[15px] lg:text-[16px] leading-[1.65] sm:leading-[1.7] space-y-2 sm:space-y-2.5 break-words [hyphens:auto] text-gray-800">
                  {s.markdown.split("\n").map((line, i) => {
                    ...
                    if (!trimmed) return <div key={`sp-${i}`} className="h-1" />;
  ...
  // Normalize pizza names for inline picker
  ...
  // Derive display strings requested (“The Works”, “Dry Stout” examples)
  const selectedPizzaDisplay =
    effectivePizzaName || "The Works";
  const chosenStyleDisplay =
    activeStyle || (suggestedStyles && suggestedStyles.length
    ...
    >
      ...
      <div className="card-body gap-4 sm:gap-5">
        ...
        <div className="flex items-center justify-between">
          <h2 className="card-title text-lg sm:text-xl font-semibold tracking-wide text-black">
            {title}
          </h2>
          <div className="flex items-center gap-2">
            {isExtracting && (
        ...
        {/* ====== VERY TOP: PIZZA DROPDOWN + SUMMARY + PAIR BUTTON ====== */}
        <div className="rounded-2xl border border-gray-200 bg-white p-4">
          <div className="text-sm font-semibold text-gray-900 mb-2">
            Choose your beer from the scanned list:
          </div>

          {/* Row 1: Pizza picker + Selected beer summary */}
            ...
            <div className="text-[13px] sm:text-sm text-gray-700">
              ...
              {pizzaNames.length ? (
                <span className="inline-block">
                  <label className="sr-only" htmlFor="pizzaSelectTop">
                    Choose a pizza
                  </label>
                  <select
                    id="pizzaSelectTop"
            ...
            <div className="text-[13px] sm:text-sm text-gray-700">
              {selectedBeerObj ? (
                ...
                <span className="text-gray-500">
                  Selected beer: none (tap “Select” on a card below)
                </span>
              )}
            </div>
          </div>

          {/* Row 2: Requested summary chips + Pair button */}
          <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              title={
                canConfirm
                  ? "Pair the selected beer with this pizza"
                  : "Pick a pizza and select a beer"
              }
            >
              Pair Beer + Pizza
            </button>
        ...
        <div className="join w-full flex-wrap sm:flex-nowrap gap-2 sm:gap-0">
          <button
            ...
            className={["btn join-item btn-sm sm:btn-md font-semibold", "transition-colors"].join(
              " "
            )}
            style={
              usingCamera
        ...
        {usingCamera && (
          <div
            ...
            style={{
              background:
                "linear-gradient(180deg, rgba(214,232,223,0.35) 0%, rgba(250,247,239,0.55) 100%)",
          ...
          >
            <div
              ...
              style={{
                background:
                  "linear-gradient(180deg, rgba(23,80,58,0.08) 0%, rgba(23,80,58,0.03) 100%)",
              }}
            >
              <video
                ref={videoRef}
            ...
            <div className="p-2 sm:p-3 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 justify-between">
              <div className="text-xs sm:text-sm text-black">
                Center the beer list and tap snap. Rear camera when available.
              </div>
        ...
        {!usingCamera && (
          <label
            ...
            style={{
              ...
              background:
                "linear-gradient(180deg, rgba(250,247,239,.7) 0%, rgba(255,255,255,.7) 100%)",
            }}
          >
            <div className="relative z-10 flex flex-col items-center gap-2 px-4 py-8 sm:py-10 text-center">
              ...
              <div className="font-medium text-sm sm:text-base text-black">
                Tap to upload a clear photo of the beer list
              </div>
              <div className="text-xs sm:text-sm text-black/70">
                Good lighting • No glare • Crop to the list if possible
              </div>
            </div>
          </label>
        )}
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-base font-semibold text-gray-900">
                Beer Results (ranked best first)
              </div>
              ...
              {!!suggestedStyles.length && (
                <div className="mt-1 text-[11px] sm:text-xs text-gray-500">
                  Scoring uses style match to: {suggestedStyles.join(", ")}.
                </div>
              )}
            </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {uniqueStyles.map((s) => (
                    <button
                      ...
                      className={[
                        "rounded-full px-3 py-1 text-xs border",
                        activeStyle.toLowerCase() === s.toLowerCase()
                          ? "bg-emerald-600 text-white border-emerald-600"
                          : "bg-white text-gray-800 border-gray-200",
                      ].join(" ")}
